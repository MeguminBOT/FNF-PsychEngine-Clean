...
function onCreate() {
    ...
    // Version text - keep fixed on screen
    trace('Creating version text...');
    psychVer = new FlxText(12, FlxG.height - 44, 0, "Psych Engine v" + MainMenuState.psychEngineVersion, 12);
    psychVer.scrollFactor.set();
    trace('psychVer created, setting format...');
    trace('FlxColor.WHITE = ' + FlxColor.WHITE);
    trace('LEFT = ' + LEFT);
    trace('OUTLINE = ' + OUTLINE);
    psychVer.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT, OUTLINE, FlxColor.BLACK);
    trace('psychVer formatted, adding to game...');
    game.addObject(psychVer);
    trace('psychVer added!');
